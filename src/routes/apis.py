import os
from src import app
from src.worker.tasks import process_user_request
...
def main():
    try:
        ...
        resp = jsonify(
            {
                "message": "The browser (or proxy) sent a request that this server could not understand. KeyError: 'file'"
            }
        )
        resp.status_code = 400
        return resp
...
def regenerate():
    ...
    if task == "abstract":
        ...
        print(
            "The task is not properly defined. Available tasks are abstract, keywords, titles."
        )
        resp = jsonify(
            "The task is not properly defined. Available tasks are abstract, keywords, titles."
        )
